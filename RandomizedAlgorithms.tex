...
\State{$n=A.len$}
\State{Break $A$ into $n/5$ sub-arrays of length 5 and sort each sub-array}
\State{Let array $C$ store the medians of the $n/5$ sub-arrays}
\State{$p=DSelect(C,n/10)$}\Comment{select ``median of medians recursively''}
\State{Partition $A$ around $p$}
\State{Let $j$ = new index of $p$, $A_1,A_2$ = 1st and 2nd part of $A$}
\If{$j==i$}\State{return $p$}
\ElsIf{$j>i$}\State{return $DSelect(A_1,i)$}
\Else\State{return $DSelect(A_2,i-j$)}
\EndIf
\EndFunction
\end{algorithmic}
\end{algorithm}
